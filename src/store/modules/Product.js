...
const actions = {
  ...
  async fetchDetailProduct({ commit }, id) {
    ...
    await axios.get(`${process.env.VUE_APP_API_URL}categories`)
        .then(res => {
  ...
  async fetchCategoryInfo({ commit }) {
    commit('setProductIsLoading', true);
    await axios.get(`${process.env.VUE_APP_API_URL}categories`)
        .then(res => {
          commit('setProductCategory', res.data);
  ...
  async loginUser({ commit }, users) {
    commit('setLoginCreating', true);
    await axios.post(`${process.env.VUE_APP_API_URL}login`, users)
        .then(res => {
          localStorage.setItem('accessToken', res.data.token)
          commit('saveNewUser', res);
          commit('setLoginCreating', false);
        }).catch(err => {
          console.log('error', err);
          commit('setLoginCreating', false);
        });
  },
  async registerUser({ commit }, users) {
    commit('setLoginCreating', true);
    await axios.post(`${process.env.VUE_APP_API_URL}register`, users)
        .then(res => {
            localStorage.setItem('accessToken', res.data.token)
            commit('saveNewUser', res);
            commit('setLoginCreating', false);
...
const mutations = {
  ...
  setLoginCreating(state, isCreating) {
    state.isCreating = isCreating
  },
